...
impl Sv39PageTable {
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        for level in (1..=2).rev() {
            ...
            let pte = self.nodes.get(&ppn).unwrap().entries[vpn_idx];
        ...
        // Level 0: 写入叶子 PTE
        let vpn_idx = Self::extract_vpn(va, 0);
        let node = self.nodes.get_mut(&ppn).unwrap();
    ...
    pub fn map_superpage(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        // 遍历 level 2
        let vpn_idx = Self::extract_vpn(va, 2);
        let node = self.nodes.get_mut(&ppn).unwrap();
        let pte = node.entries[vpn_idx];
        ...
        // Level 1: 写入叶子 PTE（大页）
        let vpn_idx = Self::extract_vpn(va, 1);
        let node = self.nodes.get_mut(&ppn).unwrap();
        node.entries[vpn_idx] = ((pa >> 12) << PPN_SHIFT) | flags;
    }
}
