...
pub fn double_in_thread(numbers: Vec<i32>) -> Vec<i32> {
    ...
    let thread=thread::spawn(move||{
        numbers.into_iter().map(|value|value*2).collect::<Vec<i32>>()
    });
...
pub fn parallel_sum(a: Vec<i32>, b: Vec<i32>) -> (i32, i32) {
    ...
    let (sum_a,sum_b)=thread::scope(|scope|{
        let res_a=a.iter().sum::<i32>();
        let res_b=b.iter().sum::<i32>();
        (res_a,res_b)
    });
    (sum_a,sum_b)
}
...
pub fn named_sleeper(value: i32, ms: u64) -> i32 {
    //done
    let bulider=thread::Builder::new()
                        .name("sleeper".into());
    let handle=bulider.spawn(move||{
        thread::sleep(Duration::from_millis(10));
        value
    }).expect("fail to spwan thread");
